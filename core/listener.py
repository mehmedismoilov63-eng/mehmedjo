...
class VoiceListener(QObject):
    ...
    # Signals
    speech_detected = pyqtSignal(str, dict)  # text, user_profile
                ...
                while self.is_listening:
                    try:
                        ...
                        if text and text.strip():
                            ...
                            self.speech_detected.emit(text, user_profile or {})
    ...
    def _convert_with_whisper(self, audio: sr.AudioData) -> Optional[str]:
        """Convert speech using faster-whisper - faqat ru tilida"""
        try:
            import numpy as np
            ...
            segs, info = self.whisper_model.transcribe(
                audio_np,
                language="ru",        # rus tilida eng aniq taniydi
                beam_size=5,
                vad_filter=True,
