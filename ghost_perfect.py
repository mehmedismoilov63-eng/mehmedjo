"""
GHOST Assistant - Perfect Professional Version
...
from core.speaker_fixed import VoiceSpeaker  # Use fixed speaker
from core.intent_parser import IntentParser
from core.context_manager import ContextManager
from core.wake_word_continuous import ContinuousWakeWordDetector
from gui.ghost_visual_simple import GhostVisualManager
...
class PerfectGhostAssistant:
    """Perfect Professional GHOST Assistant with all features"""
    ...
    def __init__(self):
        ...
        self.logger.info("Starting GHOST Assistant (Perfect Professional Version)")
    ...
    def initialize_components(self):
        ...
        try:
            ...
            self.speaker = VoiceSpeaker(self.config)  # Fixed speaker
            
            # Use fixed intents file
            self.intent_parser = IntentParser(self.config)
            self.intent_parser.intents_file = "data/intents_uz_fixed.json"
            self.intent_parser.load_intents()
    ...
    def execute_command(self, intent):
        ...
        try:
            # Greeting responses
            if action == 'greeting.hello':
                responses = [
                    "Salom! Qanday yordam bera olaman?",
                    "Assalomu alaykum! Nima qilish kerak?",
                    "Hello! How can I help you today?"
                ]
                import random
                return random.choice(responses)
                
            elif action == 'greeting.how_are_you':
                responses = [
                    "Yaxshi, rahmat! Sizchi?",
                    "I'm fine, thank you! And you?",
                    "Yaxshi, ishlayapman. Nima qilish kerak?"
                ]
                import random
                return random.choice(responses)
                
            # System commands
            elif action == 'system.volume_up':
                ...
                return ("Men quyidagi buyruqlarni bajara olaman: "
                       "salom/qalaysiz, ovozni oshir/kamaytir, skrinshot olish, "
                       "ilovalarni ochish/yopish, qidirish, eslatma berish, "
                       "vaqtni bilish, ob havoni bilish.")
    ...
    def start(self):
        ...
        print("=" * 60)
        print("📋 Buyruqlar:")
        print("   'ghost' - GHOST ni chaqirish (ovozli)")
        print("   'salom' - Salomlash")
        print("   'qalaysiz' - Qandaysiz deb so'rash")
        ...
        print("   'yordam ber' - Yordam so'rash")
        print("   'exit' - Chiqish")
        print("=" * 60)
        print()
        print("🎤 'ghost' deb ayting yoki 'salom' deb yozing")
...
def main():
    ...
    try:
        ...
        assistant = PerfectGhostAssistant()
        assistant.start()
